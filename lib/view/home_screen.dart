...
class _DiskScannerScreenState extends State<DiskScannerScreen> {
    ...
    _scannerService.scanEventsStream.listen((event) {
      ...
      switch (event['type']) {
        case 'progress':
          ...
          break;
        case 'result':
        // CHANGED: Instead of a direct add, use the new smart filtering function
          _addResult(FolderInfo(
              path: event['path'], sizeInBytes: event['size']));
          break;
        case 'complete':
          setState(() {
            _isScanning = false;
  ...
  // NEW: Smart function to prevent double counting
  void _addResult(FolderInfo newResult) {
    // Rule 1: Check if the new result is a subfolder of an existing result.
    // e.g., if list has "C:\A" and new result is "C:\A\B", ignore the new one.
    if (_scanResults.any((existing) => newResult.path.startsWith(existing.path) && newResult.path != existing.path)) {
      return; // Ignore this subfolder result
    }

    // Rule 2: Check if the new result is a parent of any existing results.
    // e.g., if list has "C:\A\B" and new result is "C:\A", remove "C:\A\B".
    _scanResults.removeWhere((existing) => existing.path.startsWith(newResult.path) && newResult.path != existing.path);

    // Add the new result (it's either unique or a new parent)
  ...
  Future<void> _toggleFolderExpansion(FolderInfo folder) async {
    ...
    folder.children = children.map((c) => FolderInfo(
      path: c.path,
  ...
  void _processDataForCharts() {
    ...
    // The sum is now correct because _scanResults has unique parent folders
    final totalFoundBytes = _scanResults.fold<int>(0, (sum, item) => sum + item.sizeInBytes);
    ...
    final userProfile = Platform.environment['USERPROFILE']?.toLowerCase() ?? '';
    _barChartData = {};
    for (var folder in _scanResults) {
      final category = _classifyFolder(folder.path.toLowerCase(), userProfile);
      _barChartData[category] = (_barChartData[category] ?? 0) + (folder.sizeInBytes / (1024 * 1024 * 1024));
    }
  }

  String _classifyFolder(String path, String userProfile) {
    if (path.contains(r'appdata')) return 'Apps';
    if (path.contains(r'steam') || path.contains(r'epic games') || path.contains(r'origin')) return 'Games';
    if (userProfile.isNotEmpty && (path.contains(r'\downloads') || path.contains(r'\documents'))) return 'Docs';
  ...
  Future<void> _openFolder(String path) async {
    ...
    if (!await launchUrl(uri)) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not open folder: $path')));
    }
  }
  ...
  void _show_about()
  {
    showAboutDialog(
      ...
      children: [
        ...
        Text(
        'This app scans your disk and shows usage statistics. Built with Flutter.',
        ),
      ],
    )]);
  }
        ...
        children: [
          Row(
            children: [
              Expanded(flex: 2, child: DropdownButtonFormField<String>(
                ...
                items: drives.map((String drive) => DropdownMenuItem<String>(value: drive, child: Text('Drive $drive'))).toList(),
                onChanged: _isScanning ? null : (String? newValue) => setState(() => _selectedDrive = newValue!),
                decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Select Drive'),
              )),
              const SizedBox(width: 16),
              Expanded(flex: 3, child: ElevatedButton.icon(
                onPressed: _isScanning ? _stopScan : _startScan,
                icon: Icon(_isScanning ? Icons.stop_circle_outlined : Icons.play_arrow),
          ...
          if (_errorMessage != null)
            Padding(
              ...
              child: Text('Error: $_errorMessage', style: TextStyle(color: Theme.of(context).colorScheme.error)),
            ),
          Padding(
            padding: const EdgeInsets.only(top: 16.0),
            child: Text('Large Folders:', style: Theme.of(context).textTheme.titleLarge),
          ),
          const Divider(),
          ListView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (context, index) {
              if(_displayList.isEmpty) {
                return Center(
                    heightFactor: 5,
                    child: Text(_isScanning ? 'Scanning...' : 'Press "Start Scan" to begin.')
                );
              }
              final folder = _displayList[index];
              return Card(
                margin: EdgeInsets.only(left: folder.level * 24.0, top: 4, bottom: 4, right: 0),
                child: ListTile(
                  leading: folder.isLoading ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 3)) : Icon(folder.isExpanded ? Icons.folder_open : Icons.folder, color: Colors.orange),
                  title: Text(folder.level > 0 ? folder.path.split(r'\').last : folder.path, style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(folder.readableSize),
                  trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                    IconButton(icon: const Icon(Icons.open_in_new), onPressed: () => _openFolder(folder.path), tooltip: 'Open in Explorer'),
                    IconButton(
                      icon: Icon(folder.isExpanded ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down),
    ...
    return AspectRatio(
      ...
      child: Card(
        child: Padding(padding: const EdgeInsets.all(12.0), child: Column(children: [
          Text('Drive: ${_totalDiskSpaceGB.toStringAsFixed(1)} GB', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          Expanded(child: PieChart(PieChartData(sections: _pieChartData, centerSpaceRadius: 0, sectionsSpace: 2))),
          const SizedBox(height: 12),
          _Indicator(color: Colors.redAccent, text: 'Found (${_foundFoldersSizeGB.toStringAsFixed(1)} GB)'),
          _Indicator(color: Colors.orange.shade700, text: 'Other Used'),
          _Indicator(color: Colors.blue.shade800, text: 'Free (${_freeDiskSpaceGB.toStringAsFixed(1)} GB)'),
        ])),
      ),
    );
  }
  ...
  Widget _buildBarChartCard() {
    return AspectRatio(
      ...
      child: Card(
        child: Padding(padding: const EdgeInsets.all(12.0), child: Column(children: [
          Text('Categories (GB)', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 16),
          Expanded(child: BarChart(BarChartData(
            alignment: BarChartAlignment.spaceAround,
            barGroups: _barChartData.entries.map((e) => BarChartGroupData(x: e.key.hashCode, barRods: [
              BarChartRodData(toY: e.value, width: 15, borderRadius: BorderRadius.circular(4), color: Theme.of(context).colorScheme.primary)
            ])).toList(),
            titlesData: FlTitlesData(
              bottomTitles: AxisTitles(sideTitles: SideTitles(
                  showTitles: true,
                  getTitlesWidget: (value, meta) => Text(_barChartData.keys.firstWhere((k) => k.hashCode == value, orElse: () => ''), style: const TextStyle(fontSize: 10))
              )),
